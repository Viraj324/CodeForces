...
public class HonestCoach {
    public static int minDiffer(int n, int[] strengths) {
        Arrays.sort(strengths);
        int mind = Integer.MAX_VALUE;
        for (int i = 0; i < n - 1; i++) {
            int currentDifference = strengths[i + 1] - strengths[i];
            if (currentDifference < mind) {
                mind = currentDifference;
            }
        }
        return mind;
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < t; i++) {
            ...
            for (int j = 0; j < n; j++) {
                strengths[j] = sc.nextInt();
            }
            int result = minDiffer(n, strengths);
